...
export function IndicatorExplorer({ symbol }: { symbol: string }) {
  ...
  }, [requestSignature, requestTargets, symbol])
  ...
  const setFamilyParam = (
    ...
    setFamilies((prev) => {
      ...
      if (family === "sma") {
        ...
        nextParams[key] = Math.round(rawValue)
        const oversold = Math.round(nextParams.oversold ?? 30)
        const overbought = Math.round(nextParams.overbought ?? 70)
        ...
        nextParams.signal = key === "signal" ? signal : Math.round(nextParams.signal ?? 9)
      }
