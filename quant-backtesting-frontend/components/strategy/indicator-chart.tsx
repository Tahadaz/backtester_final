  ...
  const figure = useMemo(() => {
    ...
    if (activeFamilies.macd) {
      ...
      traces.push({
        ...
        yaxis: axisName,
        marker: {
          color: activeFamilies.macd.indicator_values.map((value) => ((value ?? 0) >= 0 ? "#16a34a" : "#dc2626")),
        },
        opacity: 0.75,
      })
      traces.push(
    ...
    if (activeFamilies.obv) {
      ...
      traces.push(lineTrace(activeFamilies.obv.dates, activeFamilies.obv.indicator_values, "OBV", "#0891b2", axisName))
      traces.push(
        lineTrace(
