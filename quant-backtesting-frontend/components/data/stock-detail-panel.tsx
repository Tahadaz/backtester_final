    ...
    <div className="space-y-4 rounded-2xl border bg-white p-5">
      ...
      <div className="grid gap-2 sm:grid-cols-3 xl:grid-cols-6">
        <div className="rounded-xl border bg-slate-50 px-3 py-2">
          ...
          <div className="text-lg font-semibold">{summary?.present ?? 0}</div>
        </div>
        {(summary?.partial ?? 0) > 0 && (
        ...
        <div className="rounded-xl border bg-slate-50 px-3 py-2">
          ...
          <div className="text-lg font-semibold">{summary?.weekend ?? 0}</div>
        </div>
        <div className="rounded-xl border border-sky-200 bg-sky-50 px-3 py-2">
          <div className="text-[11px] uppercase tracking-wide text-sky-800">Feries</div>
          <div className="text-lg font-semibold text-sky-950">{summary?.holidays ?? 0}</div>
        </div>
        <div className="rounded-xl border border-fuchsia-200 bg-fuchsia-50 px-3 py-2">
          ...
          <div className="text-lg font-semibold text-fuchsia-950">{summary?.tentative ?? 0}</div>
        </div>
      </div>

      <div className="grid grid-cols-7 gap-2 text-[11px]">
        {["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((name) => (
          <div key={name} className="px-1 text-center font-semibold text-muted-foreground">
            {name}
          </div>
        ))}
        {cells.map((day) => {
          ...
          return (
            <div
              ...
              className={[
                "min-h-20 rounded-xl border p-2 shadow-sm",
                isPartial ? "border-yellow-400 bg-yellow-50 text-yellow-950" : stateClasses(day.state),
                inMonth ? "" : "opacity-55",
              ].join(" ")}
            >
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-semibold">{current.getDate()}</span>
                ...
                {isPartial && (
                  <span className="rounded-full bg-yellow-500 px-1.5 py-0.5 text-[9px] font-semibold text-white">
                    !
                  </span>
                )}
              </div>
              <div className="mt-2 line-clamp-3 text-[10px] leading-4">{stateLabel(day)}</div>
            </div>
          )
        })}
      </div>
      ...
      <DialogContent className="h-[95vh] max-w-[96vw] gap-0 overflow-hidden p-0 sm:max-w-[96vw]">
        {row && (
          <>
            ...
            <div className="flex-1 overflow-y-auto bg-slate-100/60 px-6 py-6">
