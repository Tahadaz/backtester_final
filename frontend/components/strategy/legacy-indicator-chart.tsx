...
type LegacyIndicatorChartProps = {
  priceSeries: LegacyIndicatorPriceSeries | null
...
export const LegacyIndicatorChart = memo(function LegacyIndicatorChart({
  priceSeries,
  priceLoading,
  priceError,
  families,
}: LegacyIndicatorChartProps) {
  ...
  const fallbackSeries =
    activeFamilies.sma ?? activeFamilies.rsi ?? activeFamilies.macd ?? activeFamilies.obv
  const baseSeries =
    priceSeries ??
    (fallbackSeries ? { dates: fallbackSeries.dates, close: fallbackSeries.close } : null)
  ...
  const figure = useMemo(() => {
    ...
    subplots.forEach((family, index) => {
      subplotAxisByFamily.set(family, `y${index + 2}` as PlotlyAxisRef)
    })
    ...
    if (activeFamilies.obv) {
      ...
      traces.push(
        lineTrace(activeFamilies.obv.dates, activeFamilies.obv.indicator_values, "OBV", "#0891b2", axisName),
      )
      traces.push(
        lineTrace(
          activeFamilies.obv.dates,
          activeFamilies.obv.indicator_overlay ?? [],
          `EMA ${activeFamilies.obv.params.ema_period ?? ""}`.trim(),
          "#f97316",
          axisName,
        ),
      )
