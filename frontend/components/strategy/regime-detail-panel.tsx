...
import type { IndicatorFamilyKey } from "./indicator-config"
...
const FAMILY_COLORS: Record<IndicatorFamilyKey, string> = {
  sma: "bg-blue-500",
  ema: "bg-teal-600",
  ema_cross: "bg-sky-500",
  ichimoku: "bg-cyan-500",
  psar: "bg-blue-700",
  macd: "bg-indigo-500",
  roc: "bg-violet-500",
  trix: "bg-purple-600",
              ...
              <div className="space-y-1.5">
                {families.map((f) => (
                  <div key={f} className="flex items-center gap-2">
                    ...
                    {weightBar(
                      data.family_weights[f] ?? 0,
                      FAMILY_COLORS[f as IndicatorFamilyKey] ?? "bg-gray-400",
                    )}
                  </div>
                ))}
              </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {Object.entries(lastFold.trending_weights).sort().map(([f, w]) => (
                        <div key={f} className="flex items-center gap-2">
                          <span className="text-[11px] font-mono w-10 uppercase">{f}</span>
                          {weightBar(w, FAMILY_COLORS[f as IndicatorFamilyKey] ?? "bg-blue-400")}
                        </div>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {Object.entries(lastFold.ranging_weights).sort().map(([f, w]) => (
                        <div key={f} className="flex items-center gap-2">
                          <span className="text-[11px] font-mono w-10 uppercase">{f}</span>
                          {weightBar(w, FAMILY_COLORS[f as IndicatorFamilyKey] ?? "bg-orange-400")}
                        </div>
                      ))}
                    </div>
                  </div>
